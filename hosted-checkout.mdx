...
```js
const res = await fetch('https://api.macleodtechnologies.org/v1/gateway/api/payment-intents', {
  method: 'POST',
  headers: { 'X-API-Key': process.env.BLUVO_KEY, 'Content-Type': 'application/json' },
  body: JSON.stringify({
    amount: '99.00',
    token: 'USDC_BASE',
    chain: 'BASE',
...
All stablecoins on all networks Bluvo supports: Solana, Ethereum, Base,
